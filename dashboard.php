<?php
// dashboard.php — Clean admin overview
session_start();
...
try {
    $stats = $pdo->query("SELECT 
        ...
        SUM(form_type='Inquiry' OR form_type IS NULL) as inquiries,
        SUM(form_type='Admission') as admissions
    FROM admission_inquiries")->fetch();
} catch (Exception $e) {
    $stats = ['total'=>0,'pending'=>0,'approved'=>0,'admitted'=>0,'rejected'=>0,'inquiries'=>0,'admissions'=>0];
}

// Today's count
try {
    $today = $pdo->query("SELECT COUNT(*) FROM admission_inquiries WHERE DATE(submission_date) = CURDATE()")->fetchColumn();
} catch (Exception $e) { $today = 0; }

// Recent 10 submissions
try {
    $recent = $pdo->query("SELECT i.*, f.faculty_name FROM admission_inquiries i LEFT JOIN faculties f ON i.faculty_id=f.id ORDER BY i.id DESC LIMIT 10")->fetchAll();
} catch (Exception $e) { $recent = []; }

// Upcoming Entrance Exams
try {
    $upcoming_exams = $pdo->query("SELECT e.*, f.faculty_name,
        (SELECT COUNT(*) FROM admission_inquiries WHERE schedule_id = e.id) as booked
        FROM entrance_schedules e 
        LEFT JOIN faculties f ON e.faculty_id = f.id 
        WHERE e.exam_date >= CURDATE()
        ORDER BY e.exam_date ASC, e.exam_time ASC LIMIT 5")->fetchAll();
} catch (Exception $e) { $upcoming_exams = []; }

// Admin Intelligence Stats
try {
    // Total Revenue Collected
    $revenue = $pdo->query("SELECT SUM(payment_amount) FROM admission_inquiries WHERE payment_status = 'Paid'")->fetchColumn() ?: 0;
    
    // Missing Documents (Admissions only)
    $missing_docs = $pdo->query("SELECT COUNT(*) FROM admission_inquiries WHERE form_type = 'Admission' AND (document_path IS NULL OR document_path = '' OR pp_photo_path IS NULL OR pp_photo_path = '')")->fetchColumn() ?: 0;
    
    // Unassigned Exams (Admissions only)
    $unscheduled = $pdo->query("SELECT COUNT(*) FROM admission_inquiries WHERE form_type = 'Admission' AND (schedule_id IS NULL OR schedule_id = 0) AND status != 'Rejected'")->fetchColumn() ?: 0;
} catch (Exception $e) {
    $revenue = 0; $missing_docs = 0; $unscheduled = 0;
}

// Today's Class-wise Distribution
$class_labels = []; $class_data = [];
try {
    $class_stmt = $pdo->query("SELECT applied_class, COUNT(*) as c FROM admission_inquiries WHERE DATE(submission_date) = CURDATE() GROUP BY applied_class");
    $classes = $class_stmt->fetchAll(PDO::FETCH_ASSOC);
    foreach ($classes as $row) {
        $class_labels[] = $row['applied_class'] ?: 'Unknown';
        $class_data[] = (int)$row['c'];
    }
} catch (Exception $e) {}

// 7-day trend
try {
    $trend_stmt = $pdo->query("SELECT DATE(submission_date) as d, COUNT(*) as c FROM admission_inquiries WHERE submission_date >= DATE(NOW() - INTERVAL 6 DAY) GROUP BY d ORDER BY d ASC");
    $trends = $trend_stmt->fetchAll(PDO::FETCH_KEY_PAIR);
...
<head>
    ...
    <style>
        ...
        .grid-stack-item-content { background: white; border-radius: 1.25rem; border: 1px solid #f3f4f6; box-shadow: 0 1px 3px 0 rgba(0, 0, 0, 0.05); overflow: hidden !important; display: flex; flex-direction: column; cursor: default; }
        ...
        .chart-container-wrap { flex: 1 1 auto; position: relative; min-height: 0; width: 100%; padding: 16px; margin: auto; }
...
<div class="max-w-[1400px] mx-auto w-full px-6 py-6">
    ...
    <div class="mb-7 flex flex-col md:flex-row md:items-start justify-between gap-4">
        <div>
            ...
            <p class="text-sm font-medium text-gray-500 mt-1.5 flex flex-wrap items-center gap-2">
    ...
    <div class="grid-stack">
        ...
        <div class="grid-stack-item" gs-id="stat-total" gs-w="3" gs-h="2" gs-x="0" gs-y="0">
            <div class="grid-stack-item-content hover:-translate-y-1 hover:shadow-lg transition-all relative drag-handle no-border group flex-col justify-center align-start p-5">
                ...
                <div class="w-11 h-11 rounded-xl bg-blue-50 text-blue-600 flex items-center justify-center mb-3 transition-transform group-hover:scale-110 shrink-0">
                ...
                <div class="text-[11px] font-bold text-gray-400 uppercase tracking-widest mt-0.5">Total Submissions</div>
            </div>
        </div>
        ...
        <div class="grid-stack-item" gs-id="stat-today" gs-w="3" gs-h="2" gs-x="3" gs-y="0">
            <div class="grid-stack-item-content hover:-translate-y-1 hover:shadow-lg transition-all relative drag-handle no-border group flex-col justify-center align-start p-5">
                <i class="edit-hint absolute top-3 right-3 text-slate-300 w-4 h-4"><svg fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6h16M4 12h16M4 18h16"></path></svg></i>
                <div class="w-11 h-11 rounded-xl bg-emerald-50 text-emerald-600 flex items-center justify-center mb-3 transition-transform group-hover:scale-110 shrink-0">
                ...
                <div class="text-[11px] font-bold text-gray-400 uppercase tracking-widest mt-0.5">Today's Forms</div>
            </div>
        </div>
        ...
        <div class="grid-stack-item" gs-id="stat-pending" gs-w="3" gs-h="2" gs-x="6" gs-y="0">
            <div class="grid-stack-item-content hover:-translate-y-1 hover:shadow-lg transition-all relative drag-handle no-border group flex-col justify-center align-start p-5">
                <i class="edit-hint absolute top-3 right-3 text-slate-300 w-4 h-4"><svg fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6h16M4 12h16M4 18h16"></path></svg></i>
                <div class="w-11 h-11 rounded-xl bg-amber-50 text-amber-600 flex items-center justify-center mb-3 transition-transform group-hover:scale-110 shrink-0">
                ...
                <div class="text-[11px] font-bold text-gray-400 uppercase tracking-widest mt-0.5">Awaiting Review</div>
            </div>
        </div>

        <!-- Stat: Admitted -->
        <div class="grid-stack-item" gs-id="stat-admitted" gs-w="3" gs-h="2" gs-x="9" gs-y="0">
            <div class="grid-stack-item-content hover:-translate-y-1 hover:shadow-xl transition-all relative drag-handle no-border gs-bg-gradient group flex-col justify-center align-start p-5">
                ...
                <div class="w-11 h-11 rounded-xl bg-white/20 backdrop-blur-sm shadow flex items-center justify-center mb-3 transition-transform group-hover:scale-110 shrink-0">
                ...
                <div class="text-[11px] font-bold text-emerald-100 uppercase tracking-widest mt-0.5">Total Admitted</div>
            </div>
        </div>
        ...
        <div class="grid-stack-item" gs-id="chart-trend" gs-w="8" gs-h="5" gs-x="0" gs-y="2">
            <div class="grid-stack-item-content">
                <div class="drag-handle relative">
                    <h3 class="text-[11px] font-extrabold text-gray-500 uppercase tracking-widest flex items-center gap-2"><svg class="w-4 h-4 text-emerald-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 7h8m0 0v8m0-8l-8 8-4-4-6 6"></path></svg> Submission Trend (7 Days)</h3>
        ...
        <div class="grid-stack-item" gs-id="chart-class" gs-w="4" gs-h="5" gs-x="8" gs-y="2">
            <div class="grid-stack-item-content overflow-hidden">
                <div class="drag-handle relative bg-gray-50/50">
                ...
                <div class="flex flex-col flex-1 relative overflow-hidden bg-gray-50/50 p-4">
                    <?php if (empty($class_data)): ?>
                        <div class="text-sm text-gray-400 font-medium text-center m-auto">No apps today.</div>
                    <?php else: ?>
                        <div class="chart-container-wrap !p-0 max-h-40 mb-3"><canvas id="classChart"></canvas></div>
                        <div class="w-full flex flex-col border-t border-gray-100 overflow-y-auto pt-3">
                            <?php foreach ($class_labels as $i => $lbl): ?>
                            <div class="flex justify-between items-center text-xs py-1.5 hover:bg-white transition-colors rounded px-2">
        ...
        <div class="grid-stack-item" gs-id="widget-breakdown" gs-w="4" gs-h="5" gs-x="0" gs-y="7">
            <div class="grid-stack-item-content overflow-hidden">
                ...
                <div class="flex-1 w-full p-4 md:p-6 flex flex-col justify-center overflow-y-auto">
                    <div class="space-y-4">
                        <div class="flex justify-between items-center group bg-gray-50 p-3 rounded-xl border border-gray-100 shrink-0">
                        ...
                        <div class="flex justify-between items-center group bg-emerald-50/50 p-3 rounded-xl border border-emerald-100 shrink-0">
                            <div class="flex items-center gap-3">
                        ...
                        <div class="w-full h-px bg-gray-100 my-1 shrink-0"></div>
        ...
        <div class="grid-stack-item" gs-id="widget-intel" gs-w="4" gs-h="5" gs-x="4" gs-y="7">
            <div class="grid-stack-item-content overflow-hidden text-center">
                <div class="drag-handle relative">
                ...
                <div class="flex flex-col gap-4 p-4 md:p-5 flex-1 overflow-y-auto bg-gray-50/50 justify-center">
                    <div class="bg-gradient-to-r from-teal-500 to-emerald-600 rounded-2xl shadow p-4 md:p-5 text-white flex items-center justify-between shrink-0">
                        <div class="text-left w-full">
                            <h3 class="text-[10px] font-bold uppercase tracking-widest opacity-80 mb-1">Total Revenue</h3>
                            <div class="text-2xl md:text-3xl font-black truncate w-full">Rs. <?php echo number_format($revenue); ?></div>
                        </div>
                    </div>
                    <div class="grid grid-cols-2 gap-4 flex-1">
                        <div class="bg-white rounded-2xl border border-rose-100 shadow-sm p-3 flex flex-col justify-center items-center text-center">
                            <div class="text-3xl font-black text-rose-500 leading-none mb-1 shadow-rose-100"><?php echo number_format($missing_docs); ?></div>
                        ...
                        <div class="bg-white rounded-2xl border border-amber-100 shadow-sm p-3 flex flex-col justify-center items-center text-center">
                            <div class="text-3xl font-black text-amber-500 leading-none mb-1 shadow-amber-100"><?php echo number_format($unscheduled); ?></div>
        ...
        <div class="grid-stack-item" gs-id="widget-exams" gs-w="4" gs-h="5" gs-x="8" gs-y="7">
            <div class="grid-stack-item-content overflow-hidden">
                <div class="drag-handle relative border-b border-gray-100">
                ...
                <div class="flex-1 w-full overflow-y-auto bg-white">
                    ...
                    <?php else: ?>
                        <ul class="divide-y divide-gray-50 h-full">
                            <?php foreach ($upcoming_exams as $ue): ?>
                                ...
                                <li class="p-4 hover:bg-gray-50 transition-colors">
                                    ...
                                    <div class="text-[11px] text-gray-400 mb-2 font-bold">
        ...
        <div class="grid-stack-item" gs-id="widget-recent" gs-w="12" gs-h="7" gs-x="0" gs-y="12">
            <div class="grid-stack-item-content overflow-hidden">
                <div class="drag-handle relative border-b border-gray-100 bg-gray-50/50">
                    <h3 class="text-[11px] font-extrabold text-gray-700 uppercase tracking-widest flex items-center gap-2"><svg class="w-4 h-4 text-emerald-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg> Daily Actions Feed</h3>
                ...
                <div class="overflow-auto flex-1 bg-white relative">
                    <table class="min-w-full text-sm divide-y divide-gray-100 relative">
                        <thead class="text-[10px] text-gray-400 uppercase tracking-wider bg-white/95 backdrop-blur sticky top-0 z-10 shadow-sm border-b border-gray-100">
                            <tr><th class="px-5 py-3.5 text-left">Student</th><th class="px-5 py-3.5 text-left">Class</th><th class="px-5 py-3.5 text-left">Date</th><th class="px-5 py-3.5 text-left">Status</th></tr>
                        </thead>
                        <tbody class="divide-y divide-gray-50">
                            ...
                            <tr class="hover:bg-slate-50 transition cursor-default">
                                <td class="px-5 py-3">
                                    ...
                                    <?php if($r['form_type']==='Admission'): ?><div class="text-[9px] font-black uppercase text-emerald-500 mt-0.5">Admission Form</div><?php endif;?>
                                </td>
                                <td class="px-5 py-3 text-gray-600 text-[11px] font-bold"><?php echo htmlspecialchars($r['applied_class']); ?></td>
                                <td class="px-5 py-3 text-gray-400 text-[11px] font-bold"><?php echo date('M d', strtotime($r['submission_date'])); ?></td>
                                <td class="px-5 py-3"><span class="px-2 py-0.5 rounded-sm text-[9px] font-black uppercase tracking-wider <?php echo $bc; ?>"><?php echo $r['status']; ?></span></td>
                            </tr>
                            <?php endforeach; ?>
                        </tbody>
                    </table>
...
editBtn.addEventListener('click', () => {
    ...
    if(isEditMode) {
        ...
        editBtn.className = "inline-flex py-1.5 px-3 bg-emerald-50 text-emerald-600 hover:bg-emerald-100 rounded-lg shadow-sm transition-all items-center gap-1.5 font-bold text-xs border border-emerald-200 shadow-md";
    } else {
